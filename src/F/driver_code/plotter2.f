      ...
      double precision chi2,chiref,delchi,prob1,mw1
...
C  choose flplot = 13 for  T  vs  S  and  neutrino data
C  choose flplot = 14 for  T  vs  S  and       M_W data
C  choose flplot = 15 for  T  vs  S  and       APV data
C  choose flplot = 16 for  T  vs  S  and     E-158 data
C  choose flplot = 17 for rho_b vs kappa_b
C
C  choose delchi = errordef in smfit.dat
      ...
      if (flplot.eq. 1) then
         ...
         open (1,file='mhmtplot/mhmt_all3.out', status='unknown')
         open (2,file='mhmtplot/mhmt_all4.out',status='unknown')
         delchi = 4.6052d0
...
c         open (1,file='mhmtplot/mhmt_asym.out', status='unknown')
c         open (2,file='mhmtplot/mhmt_asym2.out',status='unknown')
c         open (1,file='mhmtplot/mhmt_AFBbb.out', status='unknown')
c         open (2,file='mhmtplot/mhmt_AFBbb2.out',status='unknown')
c         open (1,file='mhmtplot/mhmt_ALR.out', status='unknown')
c         open (2,file='mhmtplot/mhmt_ALR2.out',status='unknown')
         open (1,file='mhmtplot/mhmt_Moller.out', status='unknown')
         open (2,file='mhmtplot/mhmt_Moller2.out',status='unknown')
      else if (flplot.eq. 6) then
         open (1,file='mhmtplot/mhmt_DIS7.out', status='unknown')
         open (2,file='mhmtplot/mhmt_DIS8.out', status='unknown')
         ...
         open (1,file='STplot/ST_E158.out', status='unknown')
         open (2,file='STplot/ST_E158_2.out',status='unknown')
      else
...
c            mtmin = 100.0d0
c            mhmin =   0.5d0
c            mtinc =  90.0d0/(9*xsteps + 2)
c            mhinc =   7.0d0/(2*ysteps - 2)
            mtmin = 110.0d0
            mhmin =   0.5d0
            mtinc =  80.0d0/(9*xsteps + 2)
            mhinc =   7.0d0/(2*ysteps - 2)
         else
     ...
     .                 mw + (mw - mw1)*(prob - chiref)/(prob1 - prob)
                  if (flplot.ge.3) write(1,100) mtp, mhmin + 2*mhinc*
     .                 (i - 1/2.d0 + (prob - chiref)/(prob1 - prob))
               endif
               if ((prob1.le.chiref).and.(prob.ge.chiref)) then
                  if (flplot.le.2) write(2,100) mtp,
     .                 mw + (mw - mw1)*(prob - chiref)/(prob1 - prob)
                  if (flplot.ge.3) write(2,100) mtp, mhmin + 2*mhinc*
     .                 (i - 1/2.d0 + (prob - chiref)/(prob1 - prob))
               endif
               prob1 = prob
               mw1 = mw
...
C            Tpar = 18.d0/452.d0*3.1d0
            ...
            do 50 i = 0, ysteps
               ...
               if (flplot.eq.16) open (5,file='STplot/plot16.dat')
               call minuit(fcn,chi2)
               close(5)
               if ((prob1.ge.chiref).and.(prob.le.chiref)) write(1,100) 
