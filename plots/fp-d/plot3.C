...
// CHECK FOR RIGHT INPUT ////////////////////////////////////////////////
   ...
   TString strpltmd = pltmd, filename = infile, strfile = infile;

   if( (strpltmd.CompareTo("cos") != 0 ) &&
       (strpltmd.CompareTo("sin") != 0 ) &&
       (strpltmd.CompareTo("tan") != 0 ) &&
       (strpltmd.CompareTo("mmp") != 0 ) ) {error(4);};
...
// CUSTOMIZE PLOT ///////////////////////////////////////////////////////
   ...
   TString plottitle = "Model: " + infile + "  |  Plot: ";

   TString xtitle, ytitle, NPleg, SMleg, display;

   NPleg = "#font[52]{M_{H}^{(NP)}, #bar{m}_{t}^{(NP)}}";
   SMleg = "#font[52]{M_{H}^{(SM)}, #bar{m}_{t}^{(SM)}}";

   if (strpltmd.CompareTo("tan") == 0) { 
   ...
   } else if (strpltmd.CompareTo("cos") == 0) { 
   ...
   } else if (strpltmd.CompareTo("sin") == 0) {
   ...
   } else if (strpltmd.CompareTo("mmp") == 0) {
...
// PREPARE BOSON MASSES AND PHI BOUNDS //////////////////////////////////
   ...
   TString mdl=filename(0,2);

   if ( (mdl.CompareTo("lr") == 0) ||
        (mdl.CompareTo("lp") == 0) ||
        (mdl.CompareTo("hp") == 0) ||
        (mdl.CompareTo("fp") == 0) ) {
      ...
      TString Higgs=filename(3,1);

      if (Higgs.CompareTo("d") == 0) {
...
// LOOP OVER ROOT FILES  ////////////////////////////////////////////////
   TGraph *NPplot, *SMplot;
   Int_t tphStep = 0;
   Int_t tphSteps;
   const int maxSteps=500;
   TMarker *NPmrk[maxSteps], *SMmrk[maxSteps];
   TString epsfile;
   ...
   for(file=0; file<=1; file++) {
      ...
      if(file==0) epsfile  =  filename + "_" + strpltmd + ".eps";
      if(file==1) filename =  filename + "_sm";
      TString rootname = filename + ".root";
   
      TFile *rootfile = TFile::Open(rootname);      
      if(rootfile == NULL) error(1);
      
      TTree *tree = (TTree*)rootfile->Get(filename);
...
// GET ARRAYS ///////////////////////////////////////////////////////////
      ...
      tphSteps = tphStep;
      const int nSteps = tphSteps;

      Float_t xArray[nSteps], yArray[nSteps], zArray[nSteps];
      ...
      for(point=0; point<Npoints; point++) {
         ...
         if( (strpltmd.CompareTo("mmp") == 0) && (tphStep==1) ) { 
      ...
      if(file==0) NPplot = new TGraph(tphSteps,xArray,yArray);
      if(file==1) SMplot = new TGraph(tphSteps,xArray,yArray);
      ...
      for(tphStep=0; tphStep<tphSteps; tphStep++){
         ...
         if( (strpltmd.CompareTo("mmp") != 0) || (tphStep !=0 )) {
         ...
         if( ((strfile.CompareTo("uu-d") == 0) || (strfile.CompareTo("nu-d") == 0)) && (strpltmd.CompareTo("sin") == 0) ) {
...
// CREATE PLOTS /////////////////////////////////////////////////////////
   ...
   if(strpltmd.CompareTo("cos") == 0) {yMin = 0.0; yMax = 1.0;}
   if(strpltmd.CompareTo("sin") == 0) {yMin = 0.0; yMax = 1.0;}
   if(strpltmd.CompareTo("mmp") == 0) {xMin = 0.0; xMax = 5.0; yMin = 0.0; yMax = 5;}
   ...
   mmleg->AddEntry(NPplot,NPleg,"l");
   mmleg->AddEntry(SMplot,SMleg,"l"); 
   mmleg->SetTextSize(0.025);
   mmleg->SetFillStyle(0);

   cout << "here" << endl;
   if( (strfile.CompareTo("uu-d") != 0) && (strfile.CompareTo("nu-d") != 0) ) {
   ...
   SMplot->Draw(display);
   mmleg->Draw();
   if( (strfile.CompareTo("uu-d") != 0) && (strfile.CompareTo("nu-d") != 0) ) s2bleg->Draw();
...
// BOUNDS ON PHI //////////////////////////////////////////////////////
   ...
   if ( (strpltmd.CompareTo("tan") == 0) || 
      ...
      for(i=0; i<100; i++) {
         ...
         phixmin0[i] = fitx;
         if (strpltmd.CompareTo("tan") == 0) { phiymin0[i] = sphmin / cphmin; phiymax0[i] = sphmax / cphmax; }
         if (strpltmd.CompareTo("cos") == 0) { phiymin0[i] = sqrt(cphmin);    phiymax0[i] = sqrt(cphmax); }
         if (strpltmd.CompareTo("sin") == 0) { phiymin0[i] = sqrt(sphmin);    phiymax0[i] = sqrt(sphmax); }
...
// LABEL ALLOWED REGION ///////////////////////////////////////////////
   ...
   if (strpltmd.CompareTo("tan") == 0) text->SetTextAngle(270);
